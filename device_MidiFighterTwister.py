...
- Switch Action Types:
	...
	- Reset Encoder Value: When Encoder Pressed, Value is reset to default value (50 with detent, 0 without)
		How It Works:
			- On a Control Change message sent to Channel 1 (Encoder Press), a value of 127 (0x7F) is sent.
			- While this message is active, the value of data2 in Channel 0 is set to 0.
			- On Release of the button, Channel 1 reverts to a value of 0.
	- Shift Encoder Toggle: Basically two encoders in one, press the encoder to change between them.
		How It Works:
			- Encoder is on Channel 0 by default (Control Changes sent via 0xB0.
			- A Control Change message of 127 is sent to Channel 1 (So the midiID is 0xB1, data2 is 0x7F)
			- This Changes the Encoder to Channel 4, where the secondary set of encoder values are (0xB4)
			- Sending another control change message to Channel 1 will send an OFF signal (0x00) to the encoder, reverting back to Channel 0.
- Encoder MIDI Type:
	- CC: Sends a CC value from 0-127
	- ENC 3FH/41H: Sends continuous value w/o keeping track of value.
...
CHANNEL DATA:
	...
	- CHANNEL 1: RGB COLOR, ENDOCDER SHIFTER
		- 0 = OFF State, 127 = ON State, any value between is considered ON, but will have a different color based on this: https://drive.google.com/file/d/0B-QvIds_FsH3Z0ZLT041VnZfOTA/view?resourcekey=0-fgp-m9PFslsMcR3SD5bJZQ
		#SendMIDI(midi.MIDI_CONTROLCHANGE, 1, 15, COLOR.ORANGE) #Change Encoder 15 to Orange (on)
	- CHANNEL 2: LED ANIMATION
	- CHANNEL 3: BANK SELECT
	- CHANNEL 4: SECONDARY ENCODER INDICATOR AMOUNT (SHIFT) (0 - 127)
	- CHANNEL 5: LED ANIMATION (AGAIN?)
	- CHANNEL 6: INDICATOR AMOUNT (AGAIN?)
	- CHANNEL 8:

LED ANIMATION CC VALUES:
...
MIDI FIGHTER TWISTER SETUP:
	...
	#Universal Mix Control (Endless Encoder)
	#event.data1 = any individual encoder/button/slider/misc; a control, data2 = CC Value (0-127)
	#if the controller being modified is the mix on the sFighterTwister (First bank, last encoder)
		#EndlessEncoder(event.data1, event.data2)
...
#eventData.status the same as eventData.midiId + eventData.midiChan
...
#				newValue = currentValue - 1
def EndlessEncoder(controller, encVal):
	if encVal == 65:
		print('MixVal + 1')
	elif encVal == 63:
		print('MixVal - 1')
